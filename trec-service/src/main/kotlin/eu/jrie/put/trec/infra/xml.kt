...
import eu.jrie.put.trec.domain.query.Topics
...
class QueriesDeserializer : StdDeserializer<Topics>(Topics::class.java) {
    override fun deserialize(p: JsonParser, ctxt: DeserializationContext): Topics {
        return p.codec.readTree<JsonNode>(p)
            .get("topic")
            .elements()
            .asSequence()
    ...
    private fun JsonNode.queryValue(): Topic {
        ...
        return Topic(id, disease, gene, treatment)
    }
}
...
val xmlMapper = XmlMapper().apply {
    val module = SimpleModule().apply {
        ...
        addDeserializer(Topics::class.java, QueriesDeserializer())
    }
